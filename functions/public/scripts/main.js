...
function FunctionsQuickstart() {
  ...
  this.addLocationButton = document.getElementById('send-Location-Button');
  ...
  this.addLocationButton.addEventListener('click', this.addLocation.bind(this));
  ...
  firebase.database().ref('/locations').limitToLast(10).on('child_added', this.onNewLocation.bind(this));
}

// Adds two numbers by calling the `addNumbers` server-side function.
FunctionsQuickstart.prototype.addLocation = function() {
  var locationTextInput = this.locationText;
  var locationText = locationTextInput.value;
  var addLocationButton = this.addLocationButton;
  addLocationButton.disabled = true;
  // [START callAddMessageFunction]
  // [START callAddMessageFunctionWithError]
  var addLocation = firebase.functions().httpsCallable('addLocation');
  addLocation({text: locationText}).then(function(result) {
    // Read result of the Cloud Function.
    var sanitizedMessage = result.data.text;
    // [END callAddMessageFunctionWithError]
    // [START_EXCLUDE silent]
    if (locationText !== sanitizedMessage) {
...
FunctionsQuickstart.prototype.onNewLocation = function(snap) {
  var text = snap.val().text;
  var name = snap.val().author.name;
  ...
  var textContainer = document.createElement('div');
  textContainer.innerText = text;
  locationElement.appendChild(authorContainer);
  locationElement.appendChild(textContainer);
  this.locationContainer.insertBefore(locationElement, this.locationContainer.firstChild);
};
